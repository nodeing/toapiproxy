...
1. 点击「复制错误报告」
2. 将剪贴板内容发给维护者
3. 如果需要完整日志，点击「打开日志目录」后发送其中的 `.log` 文件

错误报告会自动隐藏常见 API Key、token、邮箱等敏感内容。
...
在「常规」页面点击「复制错误报告」，把复制出来的内容发给维护者。需要完整文件时，点击「打开日志目录」后发送 `.log` 文件。
